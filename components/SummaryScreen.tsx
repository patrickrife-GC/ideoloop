...
export const SummaryScreen: React.FC<SummaryScreenProps> = ({ onGenerate }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8FAF9] px-6 text-center">
      <div className="max-w-xl mx-auto space-y-6 animate-fade-in-up">
        <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-sm border border-[#1f3a2e]/20">
          <svg className="w-10 h-10 text-[#1f3a2e]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
            <div 
                ...
                className={`p-4 rounded-xl border-2 cursor-pointer transition-all text-left ${selectedModel === 'GEMINI_FLASH' ? 'border-[#1f3a2e] bg-white shadow-md' : 'border-gray-200 bg-gray-50 hover:bg-white'}`}
            >
                <div className="flex items-center gap-2 mb-2">
                    <BoltIcon className={`w-5 h-5 ${selectedModel === 'GEMINI_FLASH' ? 'text-[#1f3a2e]' : 'text-gray-400'}`} />
        ...
        <div className="pt-8">
          <button
            ...
            className="rounded-full bg-[#1f3a2e] px-10 py-4 text-lg font-semibold text-white shadow-lg hover:bg-[#5a7968] hover:scale-105 transition-all w-full sm:w-auto"
          >
            Create Content
          </button>
        </div>
